...
import { friendlyAuthError } from '@/core/auth/validation';
...
export const CatLoginAnimation = ({ onBack }: { onBack: () => void }) => {
  ...
  const handleEmailChange = (text: string) => {
    setEmail(text);
    // Tự động chuyển mode phụ huynh nếu gõ ký tự @
    if (text.includes('@') && mode !== 'parent') {
      setMode('parent');
    }
  };
  ...
  const handlePressLogin = () => {
    ...
    if (!trimmed) {
      Alert.alert(
        'Thông báo',
        mode === 'parent'
          ? 'Vui lòng nhập email tài khoản phụ huynh'
          : 'Vui lòng nhập tên đăng nhập của học sinh'
      );
      return;
    }

    // Tự động nhận diện: Nếu chuỗi chứa '@' hoặc đang ở mode parent -> mở ParentPasswordModal
    if (trimmed.includes('@') || mode === 'parent') {
  ...
  const triggerLoginSequence = async (
    actorType: 'child' | 'parent',
    credential: string
  ) => {
    if (!email.trim() || !credential) return;
    ...
    formScale.value = withTiming(0, {
      duration: 600,
      easing: Easing.bezier(0.6, -0.28, 0.735, 0.045),
    });
    formTranslateY.value = withTiming(160, {
      duration: 600,
      easing: Easing.bezier(0.6, -0.28, 0.735, 0.045),
    });
    ...
    setTimeout(async () => {
      try {
        if (actorType === 'child') {
          await loginStudent({ nickname: email.trim(), pin: credential });
        } else {
          await login({
            login: email.trim(),
            password: credential,
        ...
        console.error('[CatLoginAnimation] login error:', e);
        setShowLoader(false);
        ...
        setTimeout(() => {
          const rawMsg =
            ...
            (actorType === 'child'
              ? 'Tài khoản hoặc mã PIN không chính xác.'
              : 'Email hoặc mật khẩu không chính xác.');
          const errMsg = friendlyAuthError(
            rawMsg,
            actorType === 'child'
              ? 'Tài khoản hoặc mã PIN không chính xác.'
              : 'Email hoặc mật khẩu không chính xác.'
  ...
  return (
    ...
    >
      ...
      <ParentPasswordModal
        ...
        onSubmit={(pass) => {
          triggerLoginSequence('parent', pass);
        }}
        email={email.trim()}
        title="Xin chào phụ huynh!"
        subtitle={`Nhập mật khẩu cho tài khoản ${email.trim() || ''}`}
      />
      ...
      >
        <Animated.View
          style={[catAnimatedStyle, { width: SVG_WIDTH, height: SVG_HEIGHT }]}
        >
          <Svg
          ...
          >
            <Defs>
              <LinearGradient
                ...
                x2="184.94"
                y2="172.05"
                gradientTransform="translate(-4.99 8.17) rotate(-3.61)"
              ...
              <LinearGradient
                ...
                x2="648.3"
                y2="171.89"
                gradientTransform="translate(7.49 -49.07) rotate(3.99)"
                gradientUnits="userSpaceOnUse"
              >
                <Stop offset="0" stopColor="#f7a424" />
                ...
                {!isSwallowed && (
                  <>
                    ...
                    <Path
                      ...
                      transform="translate(0, 198.87) scale(1, 0.91) translate(0, -198.87)"
          ...
          <Animated.View
            style={[
              ...
              formAnimatedStyle,
              {
                left: 254.59 * scaleRatio,
                top: 268.37 * scaleRatio,
                width: 325 * scaleRatio,
                height: 111.9 * scaleRatio,
              },
            ]}
          >
            <TextInput
              style={[
                styles.input,
                {
                  height: Math.max(44, 111.9 * scaleRatio),
                  fontSize: Math.max(15, 18 * scaleRatio),
                },
              ]}
              placeholder={
                mode === 'parent' ? 'Email phụ huynh' : 'Tên đăng nhập học sinh'
              }
          ...
          >
            ...
            <View style={styles.modeToggleContainer}>
              <TouchableOpacity
                style={[
                  styles.modeTab,
                  mode === 'child' && styles.modeTabActive,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.modeTabText,
                    mode === 'child' && styles.modeTabTextActive,
                  ]}
                >
                  🐱 Học sinh
                </Text>
              ...
              <TouchableOpacity
                style={[
                  styles.modeTab,
                  mode === 'parent' && styles.modeTabActive,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.modeTabText,
                    mode === 'parent' && styles.modeTabTextActive,
                  ]}
                >
                  👨‍👩‍👧 Phụ huynh
                </Text>
            ...
            <TouchableOpacity
              ...
              disabled={isLoading || isSwallowed}
              activeOpacity={0.85}
            >
          ...
          >
            ...
            <G transform="translate(0, -52)">
              <Path
                fill="url(#overlay-linear-gradient-7)"
              ...
              />
              <Path
                fill="url(#overlay-linear-gradient-8)"
