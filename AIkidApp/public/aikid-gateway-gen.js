 ...
 * Same contract as StoryMeeMobileApp / @storymee/sdk generateApi:
 *   POST {GATEWAY}/api/v1/media/upload  (ref drawing)
 *   POST {GATEWAY}/api/v1/jobs           jobType=image
 *   GET  {GATEWAY}/api/v1/jobs/:id      poll
 *
 * Auth: JWT from localStorage `storymee.access_token` (Expo SecureStore web fallback).
 * Billing wallet identity is derived from the verified JWT by the gateway.
...
(function (global) {
  ...
  var WALLET_USER_KEY = 'storymee.billing_user_id';
  var CHILD_KEY = 'storymee.active_child_id';
  var IMAGE_PROVIDER_KEY = 'storymee.preferred_image_provider';
  ...
  /** Peek JWT payload (no verify) — parent sub or child parentId for wallet */
  function peekJwtClaims(token) {
    try {
      ...
      var pad = json + '==='.slice((json.length + 3) % 4);
      var raw =
  ...
  /**
   * L2 wallet user id for create-credits.
   * Child JWT: parentId. Parent JWT: sub/id.
   * Fallback: localStorage written by AuthenticatedEmbed / AuthCheck.
   */
  function getBillingUserId() {
    try {
      var fromLs =
        global.localStorage && global.localStorage.getItem(WALLET_USER_KEY);
      if (fromLs) return String(fromLs).trim();
    } catch (e) {}
    var claims = peekJwtClaims(getToken());
    if (!claims) return null;
    var actor = claims.actor || claims.role || '';
    if (actor === 'child') {
      return String(claims.parentId || claims.parent_id || '').trim() || null;
    }
    return String(claims.sub || claims.id || '').trim() || null;
  }

  function getChildProfileId() {
    try {
      var fromLs = global.localStorage && global.localStorage.getItem(CHILD_KEY);
      if (fromLs) return String(fromLs).trim();
    } catch (e) {}
    var claims = peekJwtClaims(getToken());
    if (!claims) return null;
    if ((claims.actor || claims.role) === 'child') {
      return String(
        claims.childProfileId || claims.childId || claims.sub || '',
      ).trim() || null;
    }
    return null;
  }
  ...
  /**
   * Preferred image provider hint only — keys live on Hub workers.
   * gflow | vertex | gemini-native | dreamina | vidtory-sdk
   */
  function getPreferredImageProvider() {
    try {
      var p =
        global.localStorage && global.localStorage.getItem(IMAGE_PROVIDER_KEY);
      if (p && String(p).trim()) return String(p).trim();
    } catch (e) {}
    // Product default hint; server plan allowlist can remap
    return 'gflow';
  }
  ...
  async function apiJson(method, path, body, isForm) {
    ...
    if (!res.ok) {
      var msg =
        (json && (json.message || json.error || (json.error && json.error.message))) ||
        'HTTP ' + res.status;
      ...
      if (res.status === 402) {
        msg =
          json && json.message
            ? String(json.message)
            : 'Hết lượt AI tháng này. Nâng gói hoặc đợi chu kỳ mới.';
      }
      throw new Error(msg);
    }
    return json;
  ...
  async function uploadDrawingDataUrl(dataUrl) {
    ...
    var json = await apiJson('POST', '/api/v1/media/upload', fd, true);
  ...
  async function createImageJob(prompt, referenceHttpsUrl, provider) {
    var childId = getChildProfileId();
    var inputParams = {
      prompt: String(prompt || '').trim(),
      // Hint only — Hub/job-api owns Vertex/Gemini/GFlow/Dreamina credentials
      provider: provider || getPreferredImageProvider(),
    };
    if (childId) {
      inputParams.child_profile_id = childId;
    }
    ...
    inputParams.product_line = 'aikid';
    ...
    var json = await apiJson('POST', '/api/v1/jobs', body);
  ...
  async function pollJob(jobId, maxTicks, pollMs) {
    ...
    for (var i = 0; i < maxTicks; i++) {
      var json = await apiJson('GET', '/api/v1/jobs/' + encodeURIComponent(jobId));
      var job = unwrapData(json) || json;
  ...
  async function generateArtFromCanvas(dataUrl, styleName) {
    ...
    var jobId = await createImageJob(prompt, refUrl, getPreferredImageProvider());
  ...
  global.AikidGatewayGen = {
    ...
    getToken: getToken,
    getIpId: getIpId,
    getBillingUserId: getBillingUserId,
    getChildProfileId: getChildProfileId,
